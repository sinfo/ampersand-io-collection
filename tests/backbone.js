// ==================================================
//
// Tested ported from ampersand-collection-rest-mixin
// with minor alterations to work with this module
//
// ==================================================

var test = require('tape');
var AmpersandCollection = require('../ampersand-io-collection');
...
var Collection = AmpersandCollection.extend({
  model: Model,
  emit: function (event, collection, options) {
    envColl.emitArgs = {
      event: event,
...
test('Existence of methods', function (t) {
  ...
  t.ok(typeof collection.events === 'object');
  t.ok(typeof collection.listeners === 'object');
  t.ok(typeof collection.socket === 'object');
...
test('Fetch', function (t) {
  ...
  t.equal(envColl.emitArgs.event, collection.events.fetch);
  t.equal(envColl.emitArgs.collection, collection);
  t.equal(envColl.emitArgs.options.parse, true);
  t.ok(collection.listeners[collection.events.onFetch]);
  t.equal(collection.listeners[collection.events.onFetch].active, true);

  collection.fetch({parse: false});
  t.equal(envColl.emitArgs.options.parse, false);
